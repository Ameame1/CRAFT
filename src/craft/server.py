#!/usr/bin/env python3
"""
CRAFT Server - vLLM deploy wrapper with config support.

This server uses vLLM to serve the CRAFT model,
loading configuration from YAML files.

Usage:
    python src/craft/server.py
    python src/craft/server.py --config server
    python src/craft/server.py --port 8002 --model Qwen/Qwen2.5-7B-Instruct
"""
...
def main():
    parser = argparse.ArgumentParser(description="CRAFT Server")
    parser.add_argument(
        "--config", "-c",
        type=str,
        default="server",
