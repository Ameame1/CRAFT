"""
CRAFT trace judge prompt (v4): consistency-only, XML output.

v4 trace format: <reason><answer>
...
V4_JUDGE_PROMPT = """You are a strict evaluation judge for **trace consistency and auditability** in multi-hop QA.

You are given:
1) The user query
2) The retrieved documents (numbered)
3) A model output trace in v4 format: <reason><answer>
...
### (B) evidence_grounded_faithfulness
...
- v4 has no <gold_docs>, so use ONLY the document numbers cited inside <reason> as the evidence scope.
...
If <model_output> is missing any required section(s) (<reason>, <answer>), you MUST:
- Set any check that depends on the missing section(s) to 0 (cannot be audited).
- Add a failure mode: missing_reason / missing_answer.
Also:
...
Rules:
- overall_consistency = 1 iff BOTH checks are 1; else 0.
- Put 0 or more <mode> tags. Use short snake_case strings (e.g., answer_not_supported, reason_answer_contradiction, evidence_not_supported, evidence_contradicted, fabricated_claim, no_doc_references).
- Keep <rationale> concise (1-4 sentences), describing the key evidence for failures.
"""


def format_v4_judge_prompt(query: str, docs: str, model_output: str) -> str:
    return V4_JUDGE_PROMPT.format(query=query, docs=docs, model_output=model_output)
